...
import { InfrastructureConfig, CloudRegion, InfrastructureTemplate, CostEstimate, ServiceRecommendation } from './interfaces/infrastructure-config.interface';
...
export class CloudProviderManagerService {
  ...
  async selectOptimalProvider(
    ...
    if (criteria.preferredProvider) {
      if (this.providerFactory.isProviderSupported(criteria.preferredProvider)) {
        this.logger.log(`Using preferred provider: ${criteria.preferredProvider}`);
        return criteria.preferredProvider;
      } else {
        this.logger.warn(`Preferred provider ${criteria.preferredProvider} not supported`);
      }
    }

    // Get all provider comparisons
    const comparisons = await this.compareProviders(scalingPhase, config, criteria);
    
    // Sort by score (highest first)
    comparisons.sort((a, b) => b.score - a.score);
    
    const optimalProvider = comparisons[0];
    this.logger.log(`Selected optimal provider: ${optimalProvider.provider} (score: ${optimalProvider.score})`);
  ...
  async compareProviders(
    ...
    for (const providerType of providers) {
      try {
        ...
        this.logger.error(`Failed to evaluate provider ${providerType}:`, error);
      }
    }
  ...
  async getCostEstimates(
    ...
    this.logger.log(`Getting cost estimates for ${scalingPhase} phase in ${region}`);
    ...
    for (const [providerType, provider] of providers) {
      try {
        const estimate = await provider.calculateCost(scalingPhase, region, config);
        costEstimates.set(providerType, estimate);
      } catch (error) {
        this.logger.error(`Failed to get cost estimate from ${providerType}:`, error);
      }
    }
  ...
  async generateAllTemplates(
    ...
    this.logger.log(`Generating templates for ${scalingPhase} phase in ${region}`);
    ...
    for (const [providerType, provider] of providers) {
      try {
        ...
        this.logger.error(`Failed to generate template for ${providerType}:`, error);
      }
    }
   ...
   */
  async getAllAvailableRegions(): Promise<Map<CloudProviderType, CloudRegion[]>> {
  ...
  async switchProvider(
    ...
    const fromCost = await fromProviderInstance.calculateCost(scalingPhase, 'us-east-1', config);
    const toCost = await toProviderInstance.calculateCost(scalingPhase, 'us-east-1', config);
    const savings = fromCost.totalMonthlyCost - toCost.totalMonthlyCost;

    // Generate migration steps
    const migrationSteps = this.generateMigrationSteps(fromProvider, toProvider, scalingPhase);
  ...
  async validateConfigurationAcrossProviders(
    config: InfrastructureConfig
  ): Promise<Map<CloudProviderType, { valid: boolean; errors: string[]; warnings: string[] }>> {
    ...
    for (const [providerType, provider] of providers) {
      try {
        ...
        this.logger.error(`Failed to validate configuration for ${providerType}:`, error);
  ...
  private async evaluateProvider(
    ...
    const [costEstimate, recommendations, regions] = await Promise.all([
      provider.calculateCost(scalingPhase, 'us-east-1', config),
      provider.getServiceRecommendations(scalingPhase, config),
    ...
    if (criteria.costOptimization) {
      const costScore = this.calculateCostScore(providerType, costEstimate.totalMonthlyCost);
    ...
    // Performance scoring
    const performanceScore = this.calculatePerformanceScore(providerType, criteria.performanceRequirements);
    ...
    const regionScore = this.calculateRegionScore(regions, criteria.regionPreferences);
  ...
  private calculateCostScore(provider: CloudProviderType, monthlyCost: number): number {
  ...
  private calculatePerformanceScore(provider: CloudProviderType, requirements: string): number {
  ...
  private calculateRegionScore(regions: CloudRegion[], preferences: string[]): number {
